...
export const createVoiceButton = (
	...
	flow: Flow, // Añadir el flow
	textAreaDisabled: boolean,
	voiceToggledOn: boolean,
	handleToggleVoice: () => void,
	getCurrPath: () => keyof Flow | null,
	getPrevPath: () => keyof Flow | null, // Añadir getPrevPath
	goToPath: (pathToGo: keyof Flow) => void, // Añadir goToPath
	handleActionInput: (path: keyof Flow, userInput: string, sendUserInput?: boolean) => Promise<void>,
	injectMessage: (content: string | JSX.Element, sender?: string) => Promise<void>,
	streamMessage: (content: string | JSX.Element, sender?: string) => Promise<void>, // Añadir streamMessage
	openChat: (isOpen: boolean) => void, // Añadir openChat
	setInputLength: Dispatch<SetStateAction<number>>,
) => {
	return (
		<VoiceButton
			inputRef={inputRef}
			flow={flow} // Pasar flow
			textAreaDisabled={textAreaDisabled}
			voiceToggledOn={voiceToggledOn}
			handleToggleVoice={handleToggleVoice}
			getCurrPath={getCurrPath}
			getPrevPath={getPrevPath} // Pasar getPrevPath
			goToPath={goToPath} // Pasar goToPath
			handleActionInput={handleActionInput}
			injectMessage={injectMessage}
			streamMessage={streamMessage} // Pasar streamMessage
			openChat={openChat} // Pasar openChat
